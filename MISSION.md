...
- Show prompt tokens, output probabilities, logits, entropy, cache statistics, layer traces, attention-head summaries, captured hooks, memory, and numerical failures such as NaN or infinite values. Timing must use synchronized, monotonic, non-overlapping phases, separate worker processing from browser round-trip overhead, and distinguish instrumented forward-pass latency from raw serving latency.
- Expose execution methods only after runtime capability validation for the loaded model. Unsupported attribution, lens, attention, activation, intervention, or tracing surfaces should be omitted while diagnostics retain the reason.
